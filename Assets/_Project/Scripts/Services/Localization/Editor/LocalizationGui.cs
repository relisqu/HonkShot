...
namespace Scripts.Services.Localization.Editor
{
    public class LocalizationGui
    {
        ...
        public void OnGui()
        {
            ...
            {
                EditorGUILayout.LabelField($"Processing sheet {_handler.CurrentSheetIndex + 1}/{_handler.TotalSheets}");
                if (_handler.UnityWebRequest != null)
                {
                    EditorGUILayout.LabelField($"Uri: {_handler.UnityWebRequest.uri}");
                    EditorGUILayout.LabelField($"Download... {_handler.UnityWebRequest.downloadProgress:P0}");
                }
            ...
            else
            {
                ...
                EditorGUILayout.Space();
                EditorGUILayout.LabelField("Sheet IDs", EditorStyles.boldLabel);

                for (int i = 0; i < _settings.SheetIds.Count; i++)
                {
                    EditorGUILayout.BeginHorizontal();
                    EditorGUILayout.PrefixLabel($"Sheet {i + 1}: ");
                    _settings.SheetIds[i] = EditorGUILayout.TextField(_settings.SheetIds[i]);
                    if (GUILayout.Button("-", GUILayout.MaxWidth(25)))
                    {
                        _settings.SheetIds.RemoveAt(i);
                        break;
                    }
                    EditorGUILayout.EndHorizontal();
                }

                if (GUILayout.Button("+ Add Sheet", GUILayout.MaxWidth(120)))
                    _settings.SheetIds.Add("");
